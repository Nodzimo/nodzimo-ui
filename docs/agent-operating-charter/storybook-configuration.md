...
- Storybook exposes the complete repository `assets` tree under `/assets` with
  `staticDirs: [{ from: '../assets', to: '/assets' }]`. Preserve the source directory structure in public URLs:
  `assets/brand/nodzimo-symbol-icon.svg`, for example, is available at
  `/assets/brand/nodzimo-symbol-icon.svg`. Use that relative path in favicon metadata so deployments under a URL subpath
  remain supported.
- Keep project brand artwork that is not an input to the generated interface-icon pipeline under `assets/brand`.
  Storybook branding should consume the same raw brand asset instead of creating a Storybook-owned copy.
...
- Storybook-only reusable Docs blocks belong under `.storybook/blocks`. These are MDX/Docs presentation adapters, not
  package components.
- Storybook preview infrastructure that powers `.storybook/preview.tsx` belongs under `.storybook/preview-runtime`. Keep
  theme context, Docs containers, preview decorators, and preview wrappers there so `preview.tsx` stays a readable
...
- Sort top-level Storybook sections explicitly with `parameters.options.storySort` in `.storybook/preview.tsx`; do not
  ...
  `Design System`, `Core`, `Client`, then `*`, with `method: 'alphabetical'` when children should sort alphabetically.
...
- Import the Storybook stylesheet entrypoint in `.storybook/preview.tsx` with `import './preview.css'`. That CSS
  entrypoint imports full Tailwind with `source(none)`, including its application-owned Preflight. It also imports
  `../src/library.css`, which provides the same `src/theme.css` mappings published to consumers. Explicit source scans
  for `../src` and `.storybook` generate the utilities used only by stories and previews. The published package
  stylesheet intentionally omits Preflight; see [Tailwind And Styles](tailwind-and-styles.md#preflight-ownership). Do
  not import built `dist/styles.css` in local Storybook; Storybook should exercise source CSS during development.
