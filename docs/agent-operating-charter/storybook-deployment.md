...
- The build copies the complete repository `assets` tree to `storybook-static/assets`, preserving its directory
  structure at public URLs such as `https://ui.nodzimo.com/assets/brand/nodzimo-symbol-icon.svg`. Treat these paths as
  stable download URLs. Their contents and caching update with the Storybook deployment; the Storybook host is not a
  runtime dependency or a separately versioned CDN.
